...
const DoblePizarra = forwardRef(({
  ...
  useEffect(() => {
    ...
    ctxRef.current = ctx;
    drawModeloCentrado(ctx);
  }, [coordsModelo, cerrarTrazo, grosorModelo, colorModelo, rellenarModelo, suavizarModelo, posicionFigura]);
  ...
  const startDrawing = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Prevenir dibujar en el área de la figura de referencia
    const pizarraWidth = window.innerWidth / 2;
    const pizarraDibujo = posicionFigura === 'izquierda' ? 'derecha' : 'izquierda';
    
    if (pizarraDibujo === 'izquierda' && pos.x > pizarraWidth) {
      return; // No dibujar en la pizarra derecha si la figura está a la izquierda
    }
    if (pizarraDibujo === 'derecha' && pos.x < pizarraWidth) {
      return; // No dibujar en la pizarra izquierda si la figura está a la derecha
    }
  ...
  const stopDrawing = () => {
    ...
    // Enviar todas las coordenadas - la evaluación se encargará de filtrar
